...
from tensym.interpreter.token.utils import (
    WORD_TOKEN_MAP, OP_TOKEN_MAP, SET_OF_LOWER_LETTERS, SET_OF_UPPER_LETTERS, 
    SET_OF_DIGITS, word_lookup, longest_token_match_and_consume, 
    scan_newline_and_indent, Indenter, _cur_int,

    LETTERS, IDENTIFIERS, NUMERIC_CHARACTERS, WHITESPACE_CHARS, NEWLINE, 
    HASHTAG, BACKSLASH, STRING_DELIM, ENCAPSULATORS,

    unicode_to_string, is_letter_or_underscore, is_digit_or_dot, is_wordlike,
    is_end_of_iteration,

    build_token_from_digit, build_token_from_word, build_encapsulation_token,
    build_token_from_latex, build_string_token
)
...
class Lexer:
    ...
    def tokenize(
        ...
        max_iterations = max(input_length * 3, 1000)  # At least 1000 iterations for small inputs
        iteration_count = 0
        ...
        if not self.__debug:
            ...
            while iterable.current != iterable.boundary.EOI:
                ...
                try:
                    ...
                    raise RuntimeError(f"Error while tokenizing at line:\n{current}\n{e}") from e
                if token is not None:
                    self.tokens.append(token)
    ...
    def build_token(self, iterable):
        ...
        # Handle comments
        if cp == HASHTAG:
            while not is_end_of_iteration(iterable.current) and iterable.current != NEWLINE:
                iterable.advance()
        ...
        if cp in SET_OF_DIGITS or (cp == ord('.') and iterable.peek(1) in SET_OF_DIGITS):
        ...
        if remaining_chars:
            ...
            op_token_kind = longest_token_match_and_consume(temp_iter, advance_to="last")
