...
class Runtime:
    ...
    def contract(
        self, lhs: TensorValue, rhs: TensorValue, pairs: Tuple[Tuple[str, str], ...]
    ) -> TensorValue:
        ...
        return TensorValue(
            value=("contract", lhs.value, rhs.value, pairs), indices=new_indices
        )

    def raise_idx(
        self, t: TensorValue, labels: Tuple[str, ...], metric_name: Any = None
    ) -> TensorValue:
        ...
        for lbl, var in t.indices:
            if lbl in labels:
                ...
                new_indices.append((lbl, var))
        return TensorValue(
            value=("raise", t.value, labels, metric_name), indices=new_indices
        )
    ...
    def lower_idx(
        ...
        for lbl, var in t.indices:
            if lbl in labels:
                ...
                new_indices.append((lbl, var))
        return TensorValue(
            value=("lower", t.value, labels, metric_name), indices=new_indices
        )
