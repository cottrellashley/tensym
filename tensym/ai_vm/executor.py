...
from .bytecode import Instr, Op, ExecHint
...
class Executor:
    ...
    def __init__(self, backend: Backend, constants: List[Any], 
    ...
    def _execute_instr(self, instr: Instr, pc: int, code: List[Instr]) -> int:
        ...
        # Core tensor operations
            ...
            n_args = args[0]
            tensors = [self.stack.pop() for _ in range(n_args)][::-1]  # reverse for correct order
        ...
        # Differential geometry
            ...
            tensor = self.stack.pop()
            connection = self.memory.load_global(connection_slot) if connection_slot is not None else None
            result = self.backend.covariant_derivative(tensor, idx_label, connection, ctx=ctx)
            self.stack.append(result)
