...
"""
from typing import Protocol, Any, Sequence, Tuple, Optional, Union
from dataclasses import dataclass
from .values import TensorValue
from .bytecode import ExecHint
...
class Backend(Protocol):
    ...
    def tensor(self, data: Any, indices: Sequence[Tuple[str, int]], 
               ctx: Optional[BackendContext] = None) -> TensorValue:
    ...
    def tensor_product(self, *tensors: TensorValue, 
                      ctx: Optional[BackendContext] = None) -> TensorValue:
        """Compute tensor product of multiple tensors."""
        ...
    
    def contract(self, lhs: TensorValue, rhs: TensorValue, 
                pairs: Sequence[Tuple[str, str]],
                ctx: Optional[BackendContext] = None) -> TensorValue:
        """Contract tensors over specified index pairs."""
        ...
    
    def einsum(self, spec: str, *tensors: TensorValue,
              ctx: Optional[BackendContext] = None) -> TensorValue:
        """Einstein summation with explicit specification."""
        ...
    
    # Index operations
    def raise_idx(self, tensor: TensorValue, labels: Sequence[str], 
                 metric: TensorValue, ctx: Optional[BackendContext] = None) -> TensorValue:
        """Raise indices using metric."""
        ...
    
    def lower_idx(self, tensor: TensorValue, labels: Sequence[str],
                 metric: TensorValue, ctx: Optional[BackendContext] = None) -> TensorValue:
        """Lower indices using metric."""
        ...
    
    def permute(self, tensor: TensorValue, new_order: Sequence[int],
               ctx: Optional[BackendContext] = None) -> TensorValue:
        """Permute tensor indices."""
        ...
    
    # Differential geometry
    def covariant_derivative(self, tensor: TensorValue, idx_label: str,
                           connection: Any, ctx: Optional[BackendContext] = None) -> TensorValue:
        """Compute covariant derivative."""
        ...
    
    # Arithmetic operations
    def add(self, lhs: TensorValue, rhs: TensorValue,
           ctx: Optional[BackendContext] = None) -> TensorValue:
        """Add tensors."""
        ...
    
    def subtract(self, lhs: TensorValue, rhs: TensorValue,
                ctx: Optional[BackendContext] = None) -> TensorValue:
        """Subtract tensors."""
        ...
    
    def multiply(self, lhs: Union[TensorValue, float, int], rhs: TensorValue,
                ctx: Optional[BackendContext] = None) -> TensorValue:
        """Scalar-tensor or elementwise multiplication."""
        ...
    
    def negate(self, tensor: TensorValue, 
              ctx: Optional[BackendContext] = None) -> TensorValue:
        """Negate tensor."""
        ...
    
    # Simplification and optimization
    def simplify(self, tensor: TensorValue, 
                ctx: Optional[BackendContext] = None) -> TensorValue:
        """Simplify tensor expression."""
        ...
    
    def canonicalize(self, tensor: TensorValue,
                    ctx: Optional[BackendContext] = None) -> TensorValue:
...
class SymPyBackend:
    ...
    def metric(self, name: str, signature: Sequence[int]) -> TensorValue:
        return TensorValue(value=f"metric_{name}", indices=[(f"{name}_i", -1), (f"{name}_j", -1)])
    ...
    def einsum(self, spec: str, *tensors: TensorValue,
        ...
        return TensorValue(value=("einsum", spec, [t.value for t in tensors]), indices=[])
    ...
    def raise_idx(self, tensor: TensorValue, labels: Sequence[str], 
        ...
        return TensorValue(value=("raise", tensor.value, labels, metric.value), indices=new_indices)
    ...
    def lower_idx(self, tensor: TensorValue, labels: Sequence[str],
        ...
        for lbl, var in tensor.indices:
            if lbl in labels:
                ...
                new_indices.append((lbl, var))
        return TensorValue(value=("lower", tensor.value, labels, metric.value), indices=new_indices)
    
    def permute(self, tensor: TensorValue, new_order: Sequence[int],
               ctx: Optional[BackendContext] = None) -> TensorValue:
        reordered_indices = [tensor.indices[i] for i in new_order]
        return TensorValue(value=("permute", tensor.value, new_order), indices=reordered_indices)
    
    def covariant_derivative(self, tensor: TensorValue, idx_label: str,
                           connection: Any, ctx: Optional[BackendContext] = None) -> TensorValue:
        # Add the derivative index
        new_indices = tensor.indices + [(idx_label, -1)]  # covariant derivative index
        return TensorValue(value=("cov_deriv", tensor.value, idx_label, connection), indices=new_indices)
    ...
    def multiply(self, lhs: Union[TensorValue, float, int], rhs: TensorValue,
        ...
        if isinstance(lhs, (int, float)):
            return TensorValue(value=("scalar_mul", lhs, rhs.value), indices=rhs.indices)
        else:
