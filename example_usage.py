...
from tensym.ai_vm.bytecode import ExecHint, Instr, Op
...
def example_einstein_sum():
    ...
    T_type = TensorType(
        rank=2,
        indices=[("a", TensorVariance.COVARIANT), ("b", TensorVariance.COVARIANT)],
    )
...
def example_backend_swapping():
    ...
    symbols = SymbolTable()
    A_sym = symbols.define(
        "A", TensorType(rank=1, indices=[("i", TensorVariance.COVARIANT)])
    )
    B_sym = symbols.define(
        "B", TensorType(rank=1, indices=[("j", TensorVariance.COVARIANT)])
    )
