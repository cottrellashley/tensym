...
b = 2"""
        ...
        indent_tokens = [t for t in tokens if t.kind in (TokenKind.INDENT, TokenKind.DEDENT)]
...
class TestLexerEdgeCases:
    ...
    def test_many_operators_in_sequence(self):
        ...
        assert all(t.kind.name.startswith('OP_') or t.kind == TokenKind.OP_EQUATE for t in tokens)
...
"""
        ...
        assert len(non_newline_tokens) == len(expected_kinds), f"Expected {len(expected_kinds)} tokens, got {len(non_newline_tokens)}: {[t.kind for t in non_newline_tokens]}"
        for i, (token, expected_kind) in enumerate(zip(non_newline_tokens, expected_kinds)):
            assert token.kind == expected_kind, f"Token {i}: expected {expected_kind}, got {token.kind} (lexeme: {token.lexeme})"
