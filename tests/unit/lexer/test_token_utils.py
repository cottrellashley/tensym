...
def test_mixed_brackets_implicit_join():
    ...
    expI = [
        ...
        TokenKind.NEWLINE.name,
        TokenKind.INDENT.name,
        TokenKind.LBRACKET.name,
        TokenKind.RBRACKET.name,
        TokenKind.NEWLINE.name,
        TokenKind.NEWLINE.name,
        TokenKind.DEDENT.name,
        TokenKind.NEWLINE.name,
...
def test_complex_operator_sequences():
    ...
    expN = [
        TokenKind.OP_SHR_EQUAL.name,
        TokenKind.OP_SHL_EQUAL.name,
        TokenKind.OP_PLUSEQUAL.name,
        TokenKind.OP_MINUSEQUAL.name,
        TokenKind.OP_MULEQUAL.name,
        TokenKind.OP_DIVEQUAL.name,
        TokenKind.NEWLINE.name,
    ]
...
def test_unicode_mixed_with_ascii():
    ...
    comp_ops = [
        t
        for t in result
        if t in (TokenKind.OP_LE, TokenKind.OP_GE, TokenKind.OP_NE, TokenKind.NEWLINE)
    ]
    gotO = names(comp_ops)
    expO = [
        TokenKind.OP_LE.name,
        TokenKind.OP_LE.name,
        TokenKind.OP_GE.name,
        TokenKind.OP_GE.name,
        TokenKind.OP_NE.name,
        TokenKind.OP_NE.name,
        TokenKind.NEWLINE.name,
    ]
...
def test_greek_letters_and_symbols():
    ...
    expP = [
        TokenKind.KW_GREEK.name,
        TokenKind.OP_PLUS.name,
        TokenKind.KW_GREEK.name,
        TokenKind.OP_EQUATE.name,
        TokenKind.KW_GREEK.name,
        TokenKind.OP_AND.name,
        TokenKind.KW_GREEK.name,
        TokenKind.KW_RIGHTARROW.name,
        TokenKind.KW_GREEK.name,
        TokenKind.KW_SUM.name,
        TokenKind.KW_GREEK.name,
        TokenKind.KW_INT.name,
        TokenKind.KW_GREEK.name,
        TokenKind.NEWLINE.name,
    ]
