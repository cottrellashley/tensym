...
# Runners
# ---------------------------
def run_sync(tasks: int, buf_mode: str, buf: np.ndarray, per_task: int) -> tuple[list[int], float]:
    global _BUF_MODE, _BUF, _PER_TASK
    _BUF_MODE, _BUF, _PER_TASK = buf_mode, buf, per_task
...
def run_pool(tasks: int, buf_mode: str, buf: np.ndarray, per_task: int, workers: int) -> tuple[list[int], float]:
    work = task_from_specs if buf_mode == "specs" else task_from_exprs
    t0 = time.perf_counter()
    out = [None] * tasks
    with ProcessPoolExecutor(max_workers=workers, initializer=_init_worker, initargs=(buf_mode, buf, per_task)) as pool:
...
if __name__ == "__main__":
    ...
    for mode, buf in [("specs", specs_buf), ("exprs", exprs_buf)]:
        ...
        assert sync_res == pool_res, f"Mismatch between sync and pool outputs for mode={mode}"
        speedup = t_sync / t_pool if t_pool > 0 else float("inf")
        print(f"[{mode}] CPU={CPU} tasks={TASKS} per_task={PER_TASK} deg={DEG} buf_len={COUNT}")
