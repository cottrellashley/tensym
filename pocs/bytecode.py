...
def parse_program(src: str) -> Program:
    ...
    while i < len(lines):
        ...
        if ln.startswith(".BLOCK"):
            m = re.match(
                r"\.BLOCK\s+(\d+)\s+TARGET\s+([A-Za-z_]\w*)\[(\d+),(\d+),(\d+),(\d+)\]",
                ln,
            )
            if not m:
...
class VM:
    ...
    def run_block(
        self, blk: Block
    ) -> Tuple[Tuple[str, Tuple[int, int, int, int]], sp.Expr]:
        ...
        def exec_instr(op: str, args: Tuple[str, ...]):
            if op == "LDELEM":
                ...
                rd, ra, sym = args
                setreg(rd, sp.diff(getreg(ra), self.scalars[sym]))
            elif op == "CALL":
    ...
    def _call_func(self, name: str, argvals: List[sp.Expr]) -> sp.Expr:
        ...
        for op, args in fn.instrs:
            if op == "LDELEM":
                ...
                rd, ra, sym = args
                regs[rd] = sp.diff(regs[ra], self.scalars[sym])
            elif op == "RET":
                (rret,) = args
                ret = regs[rret]
                break
